...
class ValidSessionDataFilterTest {
    ...
    class WhenFilter {
        ...
        void givenValidSessionDataInRequestContext_thenRunToCompletion() {
            ...
            lenient().when(queryParameters.getFirst(UrlParams.DEVICE_TOKEN))
                    .thenReturn(VALID_DEVICE_TOKEN);
            lenient().when(queryParameters.getFirst(UrlParams.APP_VERSION))
                    .thenReturn("1.0");
            lenient().when(queryParameters.getFirst(UrlParams.DEVICE_ID))
                    .thenReturn(VALID_DEVICE_ID);
            lenient().when(queryParameters.getFirst(UrlParams.DEVICE_TYPE))
                    .thenReturn("Device");
        ...
        void givenValidSessionDataInDeviceCredentials_thenRunToCompletion() {
            ...
            when(deviceCredentials.deviceId())
                    .thenReturn(VALID_DEVICE_ID);
            when(deviceCredentials.deviceType())
                    .thenReturn("Device");
        ...
        @Test
        @DisplayName("Given incompletee session data and unknown device token, then throw BadRequestException")
