...
public class Session {
  ...
  private Session(String deviceId, String deviceType, String appVersion) {
    checkArgument(
        deviceId != null && deviceId.length() == 36,
        "Invalid or missing parameter: " + UrlParams.DEVICE_ID + ": " + deviceId);
    checkArgument(
        deviceType != null && deviceType.length() > 0,
        "Missing parameter: " + UrlParams.DEVICE_TYPE);
    checkArgument(
        appVersion != null && appVersion.length() > 0,
        "Missing parameter: " + UrlParams.APP_VERSION);
